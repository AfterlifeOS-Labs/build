...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/afterlife/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/afterlife/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the AfterlifeOS GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/afterlife/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product $release
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/afterlife/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function _lunch_meat()
{
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${AFTERLIFE_BUILD}" ]]; then
